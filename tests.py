...
class TestRecipe(unittest.TestCase):
    ...
    def test_extract_cooking_dificulty_based_on_time_to_cook_in_minutes(self):
        data = {
        ...
        result_df = (
            self.recipes
            .extract_cooking_dificulty_based_on_time_to_cook_in_minutes(df)
        )
    ...
    def test_generate_results_file(self, mock_write_csv, mock_makedirs):
        result_df = pl.DataFrame({
            "difficulty": ["Easy", "Medium", "Hard"],
            "AverageTotalTime": [30, 45, 75]
        })

        self.recipes.generate_results_file(
